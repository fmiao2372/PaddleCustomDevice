...
std::tuple<int, int, std::vector<int>> get_max_and_where_nonzero(
  ...
  for (int i = 0; i < elem_cnt; ++i) {
    if (seq_lens_encoder[i] > 0) {
      ...
      if (seq_lens_decoder[i] > 0 && seq_lens_encoder[i] + seq_lens_decoder[i] > max_seq_len_with_context) {
...
#pragma omp parallel for num_threads(OMP_THREAD_NUM)
  for (int i = 0; i < static_cast<int>(valid_batches.size()); ++i) {
    for (int j = 0; j < copy_len; ++j) {
      padded[i * padded_linewidth + j] =
          input_p[valid_batches[i] * input_linewidth + j + offsets[valid_batches[i]] / block_size];
    }
  }
}
...
std::vector<paddle::Tensor> PrepareBlockMetadata(
  ...
  auto [max_enc_len_without_context, max_enc_len_with_context, valid_batches_enc] = get_max_and_where_nonzero(
      const_cast<int*>(seq_lens_encoder_cpu.data<int>()), const_cast<int*>(seq_lens_decoder_cpu.data<int>()), max_batches_in);
  ...
  if (enc_count > 0) {
    ...
    int max_buckets_without_context = (max_enc_len_without_context + block_size - 1) / block_size;
    int max_prompt_len_without_context = max_buckets_without_context * block_size;

    auto src_padded = paddle::full({total_batch * max_prompt_len_without_context},
                                   0,
    ...
    int max_buckets_with_context = (max_enc_len_with_context + block_size - 1) / block_size;
    int max_prompt_len_with_context = max_buckets_with_context * block_size;

    auto block_list_padded = paddle::full({total_batch * max_buckets_with_context},
                                   -1,
                                   paddle::DataType::INT32,
                                   paddle::CPUPlace());
    pad_fill<int32_t>(const_cast<int32_t*>(block_tables_cpu.data<int32_t>()),
    ...
    auto block_list_hpu =
        custom_kernel::copy_tensor_wrapper(dev_ctx, block_list_padded, hpu_place);
    ...
    if (max_prompt_len_without_context == max_prompt_len_with_context) {
      ...
      for (auto b : valid_batches_enc) {
        ...
        auto seg = paddle::experimental::slice(
          rope_emb, {2}, {start}, {start + max_prompt_len_without_context}, {}, {});
